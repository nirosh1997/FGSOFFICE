function setTabelDataForRegisterStCountReport() {
    if ($('#achedamicYear').val() == "") {
        ...
        if (programmeCode != "") {
            type = 'p';
            postData = {
                action: "ForRegisterStCountReport",
...
function DataTableSetRegisterStCountReport() {
    var buttons = [
        {
            extend: 'excel',
...
function formReportStRegisterCount(dsp) {
    title = "Number of Students Registered Report";
    let str = "";
    if (dsp == "reportStRegisterCount") {
        ...
        str += '<div class="col-sm-6"><button type="button" class="btn btn-info mr-2" style="float:right" onclick="setTabelDataForRegisterStCountReport();">View Report</button></div>';
        str += '</div>';
        str += '</div>';
        str += '</div>';
        str += '</div>';
