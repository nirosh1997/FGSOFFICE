...
function DataTableForRegisterStudentsClerk() {
	...
	if ( ! $.fn.DataTable.isDataTable( '#regStudentTableClerk' ) ) 
...
function formNewSelectedStudentList(dsp) {
	...
	if (dsp == "formSelectedStudentList") {
		...
		if (dataRep['roleName'] == 'Coordinator' || dataRep['roleName'] == 'Head of the Department' || dataRep['roleName']=='Technical Coordinator') {
			...
			str += '<select class="form-control form-control-sm" id="selectedDegreeName" onchange="dataRep[this.id]=this.value;RegStudentList()">';
			str += programName;
			str += programName1;
			str += '</select>';
			str += '</div></div>';
		...
		str += '<select class="form-control form-control-sm" id="achedamicYear" onchange="dataRep[this.id]=this.value;RegStudentList()">';
		...
		str += "<div class='form-check form-check2-1'> <label class='form-check-label'> <input type='radio' id='mediumSinhala' class='form-check-input' name='selectMedium' value='mediumSinhala' onclick='dataRep[this.name]=this.value;getMediumSinhala();RegStudentList()' " + mediumSinhalaChecked + ">Sinhala<i class='input-helper'></i></label></div>";
		str += "<div class='form-check form-check2-2'> <label class='form-check-label'> <input type='radio' id='mediumEnglish' class='form-check-input' name='selectMedium' value='mediumEnglish' onclick='dataRep[this.name]=this.value;getMediumEnglish();RegStudentList()' " + mediumEnglishChecked + ">English<i class='input-helper'></i></label></div></div>";
		str += '<div class="col-12 col-sm-12 col-md-12 col-lg-3 col-xl-7"><button type="button" class="btn btn-info mr-2" style="float:right" onclick="RegStudentList();">View List</button></div>';
			...
			for (var i = rcount; i < applicationNoArr.length; i++) {
				...
				if (applicationNoArr.indexOf(applicationNoArr[i]) == applicationNoArr.lastIndexOf(applicationNoArr[i]) || (applicationNoArr.indexOf(applicationNoArr[i]) != applicationNoArr.lastIndexOf(applicationNoArr[i]) && applicationNoArr.indexOf(applicationNoArr[i]) == i)) {
					...
					if (studentNoArr[i]) {
						...
						strNew += '<td><input type="text" style="width:100%" name="studentNo' + i + '" id="studentNo' + i + '";></td>';
						var checkBoxDivIdRegistered = 'checkRegistered' + i;
						if (document.getElementById(checkBoxDivIdRegistered)) {
							if (document.getElementById(checkBoxDivIdRegistered).checked) {
								registeredChecked = 'checked';
