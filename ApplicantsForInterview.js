...
//var programName1="";
//------------------------------------------------------MAIN FUNCTION START---------------------------
function formApplicantsForInterview(dsp) {
	...
	if (dsp == "formApplicantsForInterview") {
			...
			if (dataRep['userName'] == "ssdean" || dataRep['userName'] == "ssar" || dataRep['userName'] == "Asanka") {
				...
				for (j = 0; j < degreeCodeLength; j++) {
					if (facultyCode3Arr[j] == "FAC03" & degreeTitle1Arr[j] != null) {
						if (degreeCodappeArr[j] != null) {
							if (degreeCodappeArr.indexOf(degreeCodappeArr[j]) == degreeCodappeArr.lastIndexOf(degreeCodappeArr[j]) || (degreeCodappeArr.indexOf(degreeCodappeArr[j]) != degreeCodappeArr.lastIndexOf(degreeCodappeArr[j]) && degreeCodappeArr.indexOf(degreeCodappeArr[j]) == j)) {
								programName += "<option  >" + degreeTitle1Arr[j] + "</option>";
							}
						}
					}
				}
		...
		if (dataRep['roleName'] == 'Coordinator' || dataRep['roleName']=='Technical Coordinator'  || dataRep['roleName'] == 'Head of the Department' || dataRep['roleName'] == 'Program Assistant/Clerk') {
			str += '<div class="form-group row justify-content-md-center"><div class="col-xl-5 col-md-6">';
			str += '<label for="exampleInputUsername2" class="label-r programeLable">Programme</label>';
			str += "<select class='form-control form-control-sm' id='selectedDegreeName' onchange='dataRep[this.id]=this.value;dataRep[this.id" + 1 + "]=this.value;degreecodeset(dataRep[this.id" + 1 + "]);'>";
			str += programName;
			str += programName1;
			str += '</select>';
			str += '</div>';
		...
		str += '<div class="col-xl-2 col-md-3">'
		str += '<label for="exampleInputEmail2" class="label-r yearLable" >Academic Year</label>';
		...
		str += '</select>';
		str += '</div>';
		str += '<div class="col-md-2 col-12">'

		str += '<button type="button" class="btn btn-info mr-2" style="float:right" onclick="findInterviewScreeningList();">Find</button>';
		str += '</div></div></div>';
		str += '</div></div></div>';
	}
	return str;
...
function degreecodeset(tempdegreeName) {

	for (i = 0; i < studentNoLength; i++) {
		if (degreeTitleForListDocArr[i] != null) {
			if (degreeTitleForListDocArr.indexOf(degreeTitleForListDocArr[i]) == degreeTitleForListDocArr.lastIndexOf(degreeTitleForListDocArr[i]) || (degreeTitleForListDocArr.indexOf(degreeTitleForListDocArr[i]) != degreeTitleForListDocArr.lastIndexOf(degreeTitleForListDocArr[i]) && degreeTitleForListDocArr.indexOf(degreeTitleForListDocArr[i]) == i)) {
				if (degreeTitleForListDocArr[i] == tempdegreeName) {
					var tempcode = degreeCodeForListDocArr[i];
	...
	var checkDegreeCode = tempcode;
	//alert(checkDegreeCode);
	if (checkDegreeCode.substring(0, 2) == "MA") {
		var newDegreeCode = checkDegreeCode.substring(2, 8);
		//	alert(newDegreeCode);
		tempcode = "MA/" + newDegreeCode;
	} else if (checkDegreeCode.substring(0, 4) == "MSSC") {
		var newDegreeCode = checkDegreeCode.substring(4, 8);
		//	alert(newDegreeCode);
		tempcode = "MSSC/" + newDegreeCode;
	}

	dataRep["sendapplicationNo1"] = "" + tempcode + "";
	//dataRep["sendapplicationNo1"]=tempcode;

	// dataRep["sendapplicationNo2"]=tempcode;
	// dataRep["sendapplicationNo3"]=tempcode;


	//sendForm('data4ApplicantListForInterview');
	//sendForm('data4ApplicantQualficationDetails');
	//sendForm('data4ApplicantworkDetails');


	//alert(dataRep["sendapplicationNo1"]);
	// sendForm('data4ApplicantList');
	// sendForm('data4checkdepartment');
	// sendForm('data4checkUser');
	// // alert(dataRep["sendapplicationNo"]);
...
function SelectAllList() {
	for (i = 0; i < applicationNoLength; i++) {
		...
		if (nt.getElementById('checkSdocumeelected' + i)) {
...
function CompleteApplication1() {
	...
	if (document.getElementById('selectedDegreeName').value == "Please scroll down to see the records") {
		...
		csvData.push(newStr);
		// download stuff 
		var fileName = "Applicant List.doc";
		...
		if (document.getElementById('selectedDegreeName').value == "Please scroll down to see the records") {
			...
			if (document.getElementById('searchApplicantName').value != "" & document.getElementById('searchApplicant').value != "") {
				...
				if (count >= 6) {
					...
					if (dataRep['roleName'] == 'Coordinator' || dataRep['roleName']=='Technical Coordinator'  || dataRep['roleName'] == 'Program Assistant/Clerk') {
						var viewStr1 = '<input type="button" class="btnB"  id="Download" value = "Download"  style="margin-top:0px;padding-right:20px;width:157px;background-image:url(img/wbutton.png);height:30px;" onclick = CompleteApplication1();>';
						...
						viewStr1 += '<input type="button" class="btnB" style="background-Color:#ff6633;padding-right:20px;height:30px;" value = "Return" onclick = refresshStudentList();showMenu(' + "'formCoordinatorMenu'" + ');>';
					...
					if (dataRep['roleName'] == 'Head of the Department') {
						var viewStr1 = '<input type="button" class="btnB"  id="Download" value = "Download"  style="margin-top:0px;padding-right:20px;width:157px;background-image:url(img/wbutton.png);height:30px;" onclick = CompleteApplication1();>';
						viewStr1 += '<input type = "button" class ="btnB" id="Reset" value="Reset" style="background-Color:#ff6633;padding-right:20px;height:30px;"  onclick="refresshStudentList();sendForm(' + "'data4ApplicantList'" + ');sendForm(' + "'data4checkUser'" + ');sendForm(' + "'data4checkdepartment'" + ');">';
						viewStr1 += '<input type="button" class="btnB" style="background-Color:#ff6633;padding-right:20px;height:30px;" value = "Return" onclick = refresshStudentList();showMenu(' + "'formHeadMenu'" + ');>';
						viewStr1 += '<input type = "button" class ="btnB" value="logout" style="background-Color:#ff6633;padding-right:20px;height:30px;" onclick=logOut();>';
					}
					if (dataRep['roleName'] == 'Subject Clerk' || dataRep['roleName'] == 'FGS Deputy Registrar') {
						...
						viewStr1 += '<input type = "button" class ="btnB" id="Reset" value="Reset" style="background-Color:#ff6633;padding-right:20px;height:30px;"  onclick="refresshStudentList();sendForm(' + "'data4ApplicantList'" + ')">';
						viewStr1 += '<input type="button" class="btnB" value = "Return" style="background-Color:#ff6633;padding-right:20px;height:30px;" onclick = refresshStudentList();showMenu(' + "'formClerkMenu'" + ');>';
						viewStr1 += '<input type = "button" class ="btnB" value="logout" style="background-Color:#ff6633;padding-right:20px;height:30px;" onclick=logOut();>';
					...
					if (dataRep['roleName'] == 'Assistant Registrar') {
						...
						viewStr1 += '<input type = "button" class ="btnB" id="Reset" value="Reset" style="background-Color:#ff6633;padding-right:20px;height:30px;"  onclick="refresshStudentList();sendForm(' + "'data4ApplicantList'" + ')">';
						viewStr1 += '<input type="button" class="btnB" value = "Return" style="background-Color:#ff6633;padding-right:20px;height:30px;" onclick = refresshStudentList();showMenu(' + "'formAssistantRegistrarMenu'" + ');>';
						viewStr1 += '<input type = "button" class ="btnB" value="logout" style="background-Color:#ff6633;padding-right:20px;height:30px;" onclick=logOut();>';
					...
					if (dataRep['roleName'] == 'Administrator') {
						var viewStr1 = '<input type="button" class="btnB"  id="Download" value = "Download"  style="margin-top:0px;padding-right:20px;width:157px;background-image:url(img/wbutton.png);height:30px;" onclick = CompleteApplication1();>';
						viewStr1 += '<input type = "button" class ="btnB" id="Reset" value="Reset" style="background-Color:#ff6633;padding-right:20px;height:30px;"  onclick="refresshStudentList();sendForm(' + "'data4ApplicantList'" + ')">';
						viewStr1 += '<input type="button" class="btnB" style="background-Color:#ff6633;padding-right:20px;height:30px;" value = "Return" onclick = refresshStudentList();showMenu(' + "'formAdministratorMenu'" + ');>';
						viewStr1 += '<input type = "button" class ="btnB" value="logout" style="background-Color:#ff6633;padding-right:20px;height:30px;" onclick=logOut();>';
					}
					if (dataRep['roleName'] == 'Dean') {
						var viewStr1 = '<input type="button" class="btnB"  id="Download" value = "Download"  style="margin-top:0px;padding-right:20px;width:157px;background-image:url(img/wbutton.png);height:30px;" onclick = CompleteApplication1();>';
						viewStr1 += '<input type = "button" class ="btnB" id="Reset" value="Reset" style="background-Color:#ff6633;padding-right:20px;height:30px;"  onclick="refresshStudentList();sendForm(' + "'data4ApplicantList'" + ');sendForm(' + "'data4checkdepartment'" + ');">';
